...
csv_train=(${metadata_url}/train.csv ${metadata_url}/train_clean.csv)
csv_index=()
csv_test=(${metadata_url}/test.csv ${ground_truth_url}/recognition_solution_v2.1.csv)
...
download_image_file() {
  ...
  local split_folder=$3/${idx}
  local store_folder=${split_folder}/store
  ...
  local images_tar_file_path=${store_folder}/${images_tar_file}
  local images_md5_file=md5.images_${idx}.txt
  local images_md5_file_url=${images_md5_file_base_url}/${split}/${images_md5_file}
  local images_md5_file_path=${store_folder}/${images_md5_file}
  download_file "${images_tar_file_url}" "${store_folder}"
  download_file "${images_md5_file_url}" "${store_folder}"
  validate_md5_checksum "${images_tar_file_path}" "${images_md5_file_path}"
  extract_tar_file "${images_tar_file_path}" "${split_folder}"

  rm -r "${store_folder}"
  gsutil -m cp -r "${split_folder}"/* "gs://${mybucket}/${split}"
  rm -r "${split_folder}"
}
...
download_csv_files() {
  ...
  gsutil cp "${split_folder}"/*.csv "gs://${mybucket}"
}
